...
## v0.6.4｜Unified Macro Framework｜统一大框架升级版

- 同篇文章全部章节大标题统一为 `chapter_double_rule`。
- 全部章节边界统一为单个 `short_double_divider`。
- 移除长章节 `chapter_end_signature` 与章节分隔符叠加造成的双分隔。
- 正文标识仍按语义多样化，大框架与内容层彻底分离。
- 新增标题一致性、分隔符一致性和重复章节分隔三项发布阻断门禁。
- 保留 v0.6.3 的结构前缀零泄漏修复。

## v0.6.3｜Structural Prefix Leakage Hotfix｜结构前缀泄漏热修复

- 修复 intro 段落经 `lead_groups` 渲染时绕过前缀剥离，导致 `PARALLEL_ITEM::` 出现在 HTML 的问题。
- 在计划创建和组件渲染两个边界执行双重清理，并跳过 lead 中的图片、表格内部占位符。
- 所有编译样例新增 7 类内部结构前缀零泄漏门禁。
- 标题保真回归加入“一个说法（uni-context）”和“结果却不是所有人更理解彼此。”真实问题样例。
- 保留 v0.6.2 的 94 标识、可见下划线、HTML 属性完整性和 73/17/4 启用分层。
- 纯排版边界不变：生成文案、生成标签和改写段落继续为 0。

## v0.6.2｜Visual Marker Integrity Hotfix｜视觉标识完整性热修复

- 修复实线下划线使用浅色背景作为线色，导致展示页和正文中几乎不可见的问题。
- 关键词角线描边的底线改为真实语义前景色，避免只剩左侧竖线。
- 修复标识库字体栈中的双引号破坏 HTML style 属性，恢复标题字号、居中、引用和小字等追加样式。
- 删除简单表格中的空 `style` 属性，并新增未知属性、空 style 和字体栈引号损坏阻断门禁。
- 将空心圆、菱形、两位序号、正文拉引和文章结束标识的注册表状态校准为 `content_auto`。
- 94 个标识重新执行渲染、浏览器属性、移动端溢出、视觉可见性与回归检查。
- 纯排版边界不变：生成文案、生成标签和改写段落继续为 0。

## v0.6.1｜Complete Symbol Orchestration｜完整符号编排修复

- 标识库由 84 个扩展到 94 个。
- 新增 6 个章节分隔符，并按章节自动轮换。
- 新增段首微型符号、重点句角标、逻辑递进轨道和数据组合轨道。
- 自动启用实心圆、空心圆、菱形列表、两位序号、大数字、数据徽章、拉引和文章结束标识。
- 新增无分隔符长章节、重复分隔符、装饰过载、数据标识不足和引用样式重复门禁。
- 保持标题、正文、顺序和标点逐字保真；生成文案、标签和改写段落继续为 0。

## v0.6.0｜Section Visual Orchestrator｜章节视觉编排与多标识自动组合

- 标识库由 72 个扩展到 84 个，新增 5 个章节装饰、3 个行内强调、问题组、并列句轨道、无标签原文对照和结尾聚焦框。
- 新增章节视觉编排器，每章自动组合章节入口、行内细节、段落结构、重点组件和章节收尾。
- 新增章节差异化状态：相邻章节不再重复同一视觉签名。
- 新增 `section_visual_coverage`，报告每章行内、块级、结构标识和视觉签名。
- 新增干巴章节、单标识章节、相邻重复签名和连续卡片阻断门禁。
- 事实框改为蓝色，绿色只保留给明确行动。
- 行动行内强调统一复用明确行动判断器，不再因“可以 / 开始”局部词语误标。
- 保留 v0.5.2 标题数量、文字、顺序和运行时版本门禁。
- 纯排版边界不变：生成文案、生成标签和改写段落继续为 0。
