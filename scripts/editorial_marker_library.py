...
def render_library_component(marker_id,content):
    ...
    if marker_id=='large_number_heading': return f'<section style="margin:24px 0 16px 0;"><p style="font-size:34px;line-height:1;color:{BLUE};font-weight:700;margin:0 0 8px 0;font-family:{FF};">{num}</p>{p(text,"font-size:21px;font-weight:700")}</section>'
    ...
    if marker_id=='article_end_mark': return f'<section style="margin:30px 0;text-align:center;"><p style="font-size:14px;line-height:1.5;color:{MUTED};margin:0;font-family:{FF};">◆</p>{p(content if isinstance(content,str) else text,"font-size:12px;color:#A5A5A5;text-align:center;margin-top:7px")}</section>'
    return ''
