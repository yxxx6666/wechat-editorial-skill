...
LABEL={'content_auto':'内容自动识别','manual':'手动指定','wechat_fallback':'微信降级'}
COL={'content_auto':'#2783DE','manual':'#D5803B','wechat_fallback':'#7D7A75'}
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('-o','--output',default=str(ROOT/'examples/v0.6.2-all-markers-showcase.html')); args=ap.parse_args()
 groups={}
 ...
 doc=f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v0.6.2 公众号排版-文章视觉</title><style>
...
</style></head><body><header class="hero"><div class="hero-inner"><div class="kicker">xingchen / wechat-editorial-skill</div><h1>v0.6.2 公众号排版-文章视觉</h1><p class="lead">94 个可执行渲染器。全部进入 Skill 包，但按内容自动识别、手动指定和微信降级分层启用。所有正文组件坚持原文逐字保真，不生成标签或结论。</p><div class="stats">{stats}</div></div></header><main>{''.join(cards)}</main></body></html>"""
